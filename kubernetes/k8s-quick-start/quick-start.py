...
def build_britive_client():
    ...
    if github_action_env:
        now = int(time.time())
        if now - github_token_start_time > (
            4 * 60
        ):  # if token is 4+ minutes old change it
            oidc_token = source_federation_token(
                provider="github-britive", tenant=tenant
            )
...
def create_application(application_name):
    ...
    app_id = b.application_management.applications.create(
        catalog_id=catalog_id, application_name=application_name
    )["appContainerId"]
