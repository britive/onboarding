...
def main():
    ...
    for group in data['environment_groups']:
        group_id = br.application_management.environment_groups.create(application_id=app_id, name=group['name'])['group_id']
        for env in group['environments']:
            br.application_management.environments.create(parent_group_id=group_id, application_id=app_id, description=f"k8s Enviornment for cluster {env['name']}", name=env['name'])
