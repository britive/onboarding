...
class BritiveGCP:
    def __init__(
        ...
        self.cloudresourcemanager = discovery.build(
            "cloudresourcemanager", "v1", credentials=self.credentials
        )

    def create_role(self):
        service_account_email = (
            f"{self.service_account_name}@{self.project_id}.iam.gserviceaccount.com"
        )
        ...
        try:
            ...
            if e.resp.status == 404:
                ...
                self.service.projects().serviceAccounts().create(
                    name=f"projects/{self.project_id}", body=service_account_body
                ).execute()
        ...
        try:
            role = (
                ...
                .roles()
                .create(parent=f"projects/{self.project_id}", body=custom_role_body)
                .execute()
            )
        ...
        if not any(
            ...
            and f"serviceAccount:{service_account_email}" in b.get("members", [])
            ...
            updated_policy = (
                self.cloudresourcemanager.projects()
                .setIamPolicy(resource=self.project_id, body=policy_body)
                .execute()
            )
            print(
                f"Role '{role_id}' assigned to service account '{service_account_email}'."
            )
        else:
            print(
                f"Service account '{service_account_email}' already has role '{role_id}'."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a service account role with specified parameters."
    )
    parser.add_argument(
        "-p",
        "--project_id",
        type=str,
        default="britive",
        help="The project ID for the service account.",
    )
    parser.add_argument(
        "-s",
        "--service_account_name",
        type=str,
        default="britive-service",
        help="The name for the service account.",
    )
    parser.add_argument(
        "-d",
    ...
    parser.add_argument(
        ...
        help="Name oft he role to be created for Britive integration.",
    )
    # Parse arguments
